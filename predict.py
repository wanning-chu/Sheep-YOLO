...
if __name__ == '__main__':
    # 加载模型
    model = YOLO(r'D:/deeplearning/compare_model_weights/improved_yolov8/best.pt')  # YOLOv8n模型
    model.predict(
        source=r'D:\deeplearning\PycharmProjects\ultralytics-main\sheep_dataset_second\test\分类测试集\夜间\images',
        save=True,  # 保存预测结果
        ...
        project=r'D:\deeplearning\PycharmProjects\ultralytics-main\sheep_dataset_second\test\分类测试集\测试结果图\夜间',  # 项目名称（可选）
...
#     model = YOLO(r'D:\deeplearning\compare_model_weights\improved_yolov8\best.pt')  # YOLOv8n模型
#     model.predict(
#         source=r'D:\deeplearning\PycharmProjects\ultralytics-main\sheep_dataset_second\test\images',
#         save=True,  # 保存预测结果
#         imgsz=640,  # 输入图像的大小，可以是整数或w，h
#         conf=0.25,  # 用于检测的目标置信度阈值（默认为0.25，用于预测，0.001用于验证）
#         iou=0.45,  # 非极大值抑制 (NMS) 的交并比 (IoU) 阈值
...
# if __name__ == '__main__':
#     # 加载模型
#     model = RTDETR(r'D:\deeplearning\compare_model_weights\rt_detr\best.pt')  # YOLOv8n模型
#     model.predict(
#         source=r'D:\deeplearning\PycharmProjects\ultralytics-main\sheep_dataset_second\test\images',
#         save=True,  # 保存预测结果
#         imgsz=640,  # 输入图像的大小，可以是整数或w，h
